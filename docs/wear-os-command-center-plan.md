...
11. **Structured agent-session parity is in-scope for Phase 1 (new 2026-09-23):** approve extending `session-tab-agent-status-projection.ts`'s `mobile`-client-kind filter with a capability-gated exception so paired mobile-family clients can see structured agent-session tabs, alongside terminal-backed ones, matching "all agent types." This is a runtime/mobile change that also affects Orca mobile's own phone screens, not a watch-only addition.
...
| Structured agent-session parity in Phase 1 scope                                 | Dhiman and Orca implementation owner | Approval of new decision 11; mobile-side projection-filter change reviewed                        | Open — new 2026-09-23              |
...
**1. An untracked prototype at root `wear/` is a working implementation of this plan's rejected direct-client design, holding real mobile-scope runtime authority — not a partial start on the approved companion architecture.** `wear/` is a second, independent Expo app with its own `pnpm-workspace.yaml`/`pnpm-lock.yaml`. Concrete, file:line-grounded findings:
   ...
   This worker (and the independent reviewer) did not modify, delete, or otherwise touch `wear/`; the instruction to preserve preexisting dirty/untracked files applies. Resolution — archive outside the repo, delete, or explicitly re-scope as its own separately approved initiative — is Dhiman's decision; this plan applies none of those options and does not treat any part of `wear/` as a starting point for Phases 0-3 below. Positive, reusable *product-mechanics* observations (not architecture): the pairing-code UX, an accepted/rejected/unknown send vocabulary that correctly avoids auto-retry on ambiguity, a round-screen-aware inset calculation, and a `WearButton` that meets the 48×48dp floor in source (not measured on a physical round display) are worth knowing about if a future, separately approved initiative ever revisits the direct-client model — they are not applicable to this plan's companion architecture, whose transport, crypto, token model, package identity, and theme are all different by construction.

**2. Wear OS 7 is now a general-availability release, not a preview.** Re-checked 2026-09-23 against `developer.android.com/training/wearables/versions/7/changes`: Wear OS 7 runs Android 17 (API level 37) and is GA, having shipped 2026-06-16 — reclassified from this plan's earlier A2/preview citation to A1. This does not change the approved API 33 floor (decision 8 still stands as the minimum); it only means Phase 0's device-inspection step must not assume the watch is on Wear OS 6.
...
- **Structured agent-session parity requires one bounded, in-scope runtime change, not a deferral.** `src/main/runtime/rpc/methods/session-tab-agent-status-projection.ts:16-24` hard-filters structured `agent-session` tabs out of every `session.tabs.subscribeAll`/`listAll` payload whenever `clientKind === 'mobile'`, before any Wear-specific code runs — Orca mobile's own phone screens cannot see them today either. Reaching "all agent types" therefore requires a capability-gated exception to that filter, scoped to the `mobile` client kind, landing in Phase 1 alongside the rest of the session-inventory work (see the Phase 1 plan and decision register) — not a watch-only workaround, since the watch has no data source the phone doesn't already have, and not an indefinitely deferred non-goal either. The watch and Orca mobile's own phone screens gain this together.
...
- Open a selected agent's recent native-chat conversation (terminal-backed today; structured once Phase 1's mobile-parity change lands) and send a short typed or dictated message through the atomic host method, with a real end-to-end reply round trip — not a stub that only renders a local echo.
...
An extracted `wear/` repository, on its own, is not a working product: Google Data Layer requires the watch and its paired phone app to share an application ID and installed signing certificate (decision 3), so a plucked-out watch codebase cannot interoperate with an arbitrary Orca phone install unless a compatible signed phone counterpart is published and maintained alongside it — extraction is a two-sided distribution problem, not just a source-code boundary. This plan does not solve that now; it deliberately avoids building a generic multi-backend transport framework for a hypothetical future consumer, keeping one small versioned phone/watch contract and one Orca-specific adapter (this section). Whenever extraction is actually pursued, it needs its own decisions for shipping-artifact/phone-bridge ownership, coordinated application-ID/signing configuration (no keys ever committed to either repository), and a reproducible paired-build smoke test proving the extracted watch app and its phone counterpart still interoperate after the split — named here as a future prerequisite, not undertaken by this document.
...
4. Extend `session-tab-agent-status-projection.ts`'s `clientKind !== 'mobile'` structured-visibility check (line 16) with a capability-gated exception (decision 11) so a paired mobile-family client that declares the new capability sees structured `agent-session` tabs identically to a desktop/runtime client, alongside terminal-backed tabs. This lands once, in the runtime, and benefits Orca mobile's own phone screens too — it is not a watch-only branch. Old runtimes and clients that never declare the capability keep today's terminal-only behavior.
...
- `src/main/runtime/rpc/methods/session-tab-agent-status-projection.ts` (decision 11): extend the `clientKind !== 'mobile'` structured-visibility check (line 16) with the capability declared above, so a mobile-family client that opts in sees structured `agent-session` tabs; unchanged for clients that don't declare it
...
- Structured `agent-session` tabs render identically to terminal-backed tabs for a capability-declaring client, matching what Orca mobile's own phone screens show once decision 11 lands; a client that never declares the capability is unaffected.
