...
**Current acceptance checkpoint:** The direct watch grant, compact dashboard, local/WSL and structured conversation/reply paths, bounded all-pairing fleet inventory, and redacted in-app host event feed are implemented in source. An isolated encrypted runtime paired with the Wear emulator; the watch displayed Attention, recovered its pairing after app process restart, and replayed a redacted host event. Remaining software gates are usable non-ADB enrollment, SSH exact-target reply with durable host receipts, fresh usage after cold start, and timely host-originated background notifications with screen-off evidence. Android recommends FCM for standalone Wear notifications; this repo has no Firebase configuration, so that external setup is pending. Physical Watch8 pairing, reply, installed APK identity, and all-fleet acceptance remain unverified while the device is absent from ADB. Builds and emulator success do not close those gates.
