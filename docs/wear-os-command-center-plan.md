...
| Field              | Value                                                                                                                                             |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| Status             | **Pending approval for Phase 1+ product implementation — Phase 0A/0B isolated feasibility spike Dhiman-authorized and executed 2026-09-23 (see the spike evidence log and Verification Report); physical-device gates remain blocked on device connection** |
...
- **Conflicting snapshots are labeled, not summed.** Multiple hosts contributing to one verified group can observe the same underlying account at different freshness. `providerUsage` reflects the most recently `updatedAt` reading among `sourceHostIds`; percentages and window figures across sources are never averaged or added together, and the UI surfaces which host the shown reading came from (via `sourceHostIds`) rather than presenting a blended number as if it were one authoritative live read.
...
**Environment discovery.** JDK 17.0.20 (Microsoft build) at `C:\Program Files\Microsoft\jdk-17.0.20.101-hotspot`; Android SDK at `%LOCALAPPDATA%\OrcaDevTools\AndroidSdk` with platforms `android-31/33/36` and build-tools `35.0.0`/`36.0.0` installed; `adb` 1.0.41 present. `adb devices -l` returned an empty device list both at spike start and re-checked at the end — **no physical device or emulator is connected**. Seven pre-configured AVDs exist (`orca_phone_api31_e2e`, `orca_phone_api31_play`, `orca_phone_api36_e2e`, `orca_wear4_large_round`, `orca_wear4_small_round`, `orca_wear6_large_round`, `orca_wear6_small_round`) but none were booted — pairing a phone and watch emulator through Data Layer is real device-level product verification, not a bounded static/build probe, and stays out of this spike's scope.
...
**Companion Data Layer/headless probe on emulators (2026-09-23, bounded ~15 minutes, follow-up spike authorized after this log's evidence was reviewed).** Checked host resources first (7.9 GB free RAM of 31.4 GB, 29% CPU load, no `qemu`/emulator process already running) before booting anything. Booted exactly one phone AVD (`orca_phone_api31_e2e`, matching the approved API 31 floor) and one Wear AVD (`orca_wear4_small_round`, matching the API 33 floor) headless (`-no-window -no-audio -no-boot-anim`), with no `-wipe-data` and no alteration of either AVD's stored user data; both reached `sys.boot_completed=1` within roughly a minute — faster than expected, leaving real time for the probe itself. Confirmed Google Play Services present on both (`com.google.android.gms` versionName 21.24.23 phone / 23.36.12 watch) and Bluetooth enabled on both (`bluetooth_on=1`). Checked for a path to establish a Data Layer node connection between the two emulators without physical hardware: `adb shell pm list packages` on the phone emulator returned zero Wear-OS-related packages (no `com.google.android.apps.wear.companion` or similar) — only the generic `com.android.companiondevicemanager` system service; a search of the local Android SDK installation for a bundled Wear-OS emulator pairing-assistant APK (the mechanism Android Studio's Device Manager uses to pair a Wear emulator with a phone emulator) found none. **Concrete blocker, reached by direct attempt, not assumed:** Wear OS emulator-to-phone-emulator companion pairing is mediated by Android Studio's own pairing assistant (bundled with the IDE, not the command-line SDK/`adb` tooling this environment has), and the phone emulator has no Wear OS companion app installed to receive a pairing handshake even if one were attempted manually; Play Store is not available to install one in this headless configuration. This blocks establishing a Data Layer node connection between two emulators from this CLI-only environment specifically — it does not require physical hardware to *compile or install* a companion probe, only to *pair* one, and a custom minimal companion pair (the ReactNative/Expo native-listener-plus-headless-task app this task asked to build) was not attempted beyond this point once the pairing prerequisite itself was confirmed missing, to stay inside the bounded time window rather than build an app that could not be paired to prove anything once built. Both emulators were then cleanly shut down (`adb emu kill`) to free host resources; `adb devices` confirmed both gone.

**Deeper pairing investigation + real minimal companion probe (2026-09-23, follow-up after Dhiman directed exhausting emulator-first work before asking for physical hardware).** Confirmed Android Studio is not installed on this machine (`C:\Program Files\Android\Android Studio` absent), ruling out its bundled pairing assistant as an option here, not merely inferring that from a missing APK. Fetched Google's own docs (`developer.android.com/training/wearables/get-started/connect-phone`): they document only the Android Studio Device Manager "Pair Wearable" GUI flow, requiring a phone on API 30+ with Google Play Store and the Wear companion app installed; no CLI-only pairing method is documented anywhere by Google. Per this task's direction, tried the Play-capable AVD (`orca_phone_api31_play`) instead of the bare image: it does ship `com.google.android.apps.wear.companion` preinstalled (the real companion app) and already has a signed-in Google account from prior AVD state — no login or credential handling was performed by this pass. Found a promising `AUTOMATED_SETUP` intent-filter/activity declared in that app's manifest, but every attempt to invoke it (`am start -a ...AUTOMATED_SETUP`, `am start -n .../.core.application.RootActivity`) failed with "Activity class does not exist" or "unable to resolve Intent," and `adb shell pm enable` on the component was rejected outright with `SecurityException: Shell cannot change component state`. This is a hard OS-level restriction, not a missing-package inference: the real companion app is present and its automation entry point is declared, but a plain `adb shell` session was denied with a `SecurityException` when it tried to activate that component. Exactly what elevated channel Android Studio's Device Manager uses instead is not established here — that would require inspecting Studio's own source, out of scope for this bound — only that this session's route to it is blocked, and no attempt was made to bypass that denial.

In parallel, built the minimal native companion probe this task asked for, at `spikes/wear-companion/` (source only, no build output or keystores committed — see that directory's `README.md`): two Gradle modules, `phone/` and `watch/`, sharing `applicationId com.orcaspike.companion` and a common debug-signing key, each with a role-specific manifest (watch adds `android.hardware.type.watch` and `standalone=false`; phone does not). `phone/MainActivity.kt` enumerates connected Data Layer nodes and sends a fixed `PING` (`requestId=orca-spike-request-0001`) to each, logging accepted/rejected/unknown per node rather than an optimistic "sent." `watch/PingListenerService.kt` is a `WearableListenerService` (the native primitive a Headless JS task would wrap) that acks with `PONG` on receipt and logs a duplicate-request case without re-processing. Built with `./gradlew :phone:assembleDebug :watch:assembleDebug` (AGP 8.7.2, Kotlin 2.1.20, matching this machine's cached toolchain) from an isolated temp copy: **exit 0** for both. Installed both (`adb install`, both `Success`) on the same two emulators. The watch app launched cleanly: logcat shows `I OrcaSpikeWatch: native entry reached: MainActivity.onCreate` — real, direct evidence that native entry, install, and cold start work on this toolchain outside the rejected prototype. The phone app's install also reported `Success`, but `am start -n com.orcaspike.companion/.MainActivity` failed both on first launch and after a clean uninstall/reinstall with `Error type 3: Activity class {com.orcaspike.companion/com.orcaspike.companion.MainActivity} does not exist` — despite the activity being correctly listed in `dumpsys package`'s intent resolver table and both `classes.dex`/`classes2.dex`/`classes3.dex` present in the built APK. This is a real, reproducible bug in the phone module specifically (the watch module, same toolchain and pattern, launched without issue) that this bounded pass did not have time to root-cause further — most likely related to the phone module's larger multidex-split dependency graph (Play Services Wearable plus AndroidX Appcompat) versus the watch module's minimal one. Because the phone app never reached `onCreate`, the `PING`/`PONG`/duplicate-ack logic was never exercised end-to-end; only the watch-side native listener registration and cold-start path were validated. Both emulators were shut down cleanly afterward and the isolated build directory removed; nothing in `spikes/wear-companion/` or the original AVDs was altered beyond normal app install/uninstall on the already-booted test emulators.

**Phone launch-bug diagnostic (2026-09-23, immediate follow-up; 15-minute bound).** Re-booted the bare `orca_phone_api31_e2e` (not the Play-capable AVD used in the pairing investigation above) and `orca_wear4_small_round`, confirming actual serials by device characteristics (`ro.build.characteristics`) rather than assumed boot order — this run happened to assign them in the opposite order from before, which is exactly the kind of mixup worth checking rather than assuming. Rebuilt both modules clean (exit 0) and reinstalled: the phone app's `MainActivity` launched successfully on the first attempt, and continued to launch cleanly across a force-stop/relaunch cycle and an immediate uninstall→reinstall→`am start` chain with no delay — logcat confirms native `onCreate` every time, plus a genuinely useful new signal: `connectedNodes` now fails with a specific, real error (`API: Wearable.API is not available on this device`) instead of the vague "no nodes" this log previously only reasoned about, because this bare AVD lacks the Play Services Wearable component the Play-capable AVD has. The earlier failure did not reproduce; the most likely explanation is that the earlier session had switched to installing onto the Play-capable AVD (heavier system-app set, real Play Protect scanning, a signed-in account) partway through its pairing investigation without this log stating so explicitly at the time — named here as a plausible, unconfirmed hypothesis, not a proven root cause, since that specific AVD was not re-tested this pass. Also extracted the PING/PONG request-correlation/dedupe logic out of `PingListenerService` into a pure `PingRequestDeduper` class and added 4 local JVM unit tests (`spikes/wear-companion/watch/src/test/.../PingRequestDeduperTest.kt`) covering first-seen, exact-duplicate, distinct-ID, and last-seen-only-not-full-history cases; `:watch:testDebugUnitTest` reports 4 tests, 0 failures, 0 errors. These are explicitly local logic tests, not cross-device evidence — the file and this log both say so. Verified package/signing/manifest correctness by direct inspection rather than assumption: `aapt2 dump badging` and `apksigner verify --print-certs` confirm both APKs share `applicationId com.orcaspike.companion` and an identical debug-signing SHA-256; source review confirms only the intended components (`phone/MainActivity`, `watch/MainActivity`, `watch/PingListenerService`) are `exported="true"`, and the `/orca-spike/ping` → `/orca-spike/pong` path pair matches exactly between the phone sender, the watch manifest's intent-filter `pathPrefix`, the listener's path check, and the phone's reply-path check. Genuine Data Layer pairing and an actual cross-device PING/PONG round trip remain unproven — nothing here fakes that. Emulators shut down cleanly; only the isolated temp build copy was removed.

**Spike gate matrix:**

| Gate | State |
| --- | --- |
| Toolchain (JDK/SDK/adb/AVDs) discovered | **Closed** — evidence above |
| Structured-session RPC surface audited with source lines | **Closed** — evidence above; decision 11 scope corrected |
| Applicable existing tests exercised | **Closed** — 61 tests, 0 failures, evidence above |
| Package/signing identity inspected (public fingerprints) | **Closed** — applicationId mismatch confirmed with line evidence; debug-key coincidence unexplained (correction above); no real release cert available to compare |
| Rejected `wear/` prototype's toolchain-compile feasibility | **Closed** (that codebase's toolchain portability only) — real APK produced in isolated copy; not companion-architecture evidence |
| Extraction-ready standalone build (JS/TS layer) | **Closed** — install/typecheck/test/lint all pass with zero siblings |
| Documented durable-write algorithm sanity-checked at modeled step boundaries | **Reasoned, lightly Grounded only** — not production-code evidence, not process-kill evidence, not power-cut evidence (correction above) |
| Windows durable-write power-cut durability | **Blocked** — no hardware fault-injection available; decision 9 stays fully open |
| Emulator-to-emulator companion Data Layer pairing | **Blocked — confirmed with the real companion app present, not inferred from its absence**: Play-capable AVD does ship `com.google.android.apps.wear.companion`, but its automation entry point rejects activation from plain `adb shell` with `SecurityException`; Google's own docs confirm only the Android Studio GUI flow exists (not installed on this machine) (evidence above) |
| Minimal companion probe: build, install, native entry (watch) | **Closed** — `assembleDebug` exit 0 for both modules; watch installed and launched, logcat confirms native `onCreate` reached; source at `spikes/wear-companion/` |
| Minimal companion probe: phone-side native entry | **Closed 2026-09-23** — did not reproduce on the bare `orca_phone_api31_e2e` AVD across cold-install, force-stop/relaunch, and immediate uninstall/reinstall cycles; logcat confirms native `onCreate` every time (evidence above) |
| Minimal companion probe: package identity, signing, exported components, PING/PONG path consistency | **Closed** — `aapt2`/`apksigner` confirm matching `applicationId com.orcaspike.companion` and identical debug-signing SHA-256 on both APKs; source inspection confirms the watch's `MainActivity`/`PingListenerService` and the phone's `MainActivity` are the only `exported="true"` components, and the `/orca-spike/ping`→`/orca-spike/pong` path pair matches exactly between sender and listener |
| Local correlation/dedupe logic, independent of pairing | **Closed** — `PingRequestDeduper` extracted as pure logic and covered by 4 local JVM unit tests (`:watch:testDebugUnitTest`, exit 0, 4/4 passed, 0 failures); explicitly labeled in-source as an injected/local test, not cross-device evidence |
| Minimal companion probe: PING/PONG round trip over a real Data Layer connection | **Blocked** — depends entirely on the still-unresolved companion pairing gate above; the phone app's own `connectedNodes` query now runs and returns a specific, real error (`Wearable.API is not available on this device`) on the bare AVD rather than crashing or hanging, which is itself informative but is not a round trip |
| Physical Samsung watch / phone pairing, Data Layer round trip, Headless JS wake, TalkBack, rotary, battery | **Blocked — physical device unavailable** (`adb devices` empty at every check this spike made); distinct from the emulator-pairing blocker above, and not a stand-in excuse for not having attempted the emulator path |
| Real release/Play signing certificate comparison | **Blocked** — no release-signed artifact exists in this checkout |

**Next action:** three independent paths remain, and any one closes real evidence this environment could not reach on its own — (1) fix the phone module's native-entry bug (most likely multidex-related given the working watch module's simpler dependency graph) and re-run the round trip on the two already-proven-bootable AVDs, entirely without new tooling; (2) a person with Android Studio installed pairs the two emulators through its Device Manager (now confirmed as the only documented path, with a `SecurityException` ruling out a CLI workaround), after which the same CLI tooling used here could drive the probe; or (3) connect the target Samsung Galaxy Watch (and, for a full companion round trip, a paired Android phone) via `adb`/ADB-over-Bluetooth or Wi-Fi debugging, and re-run this spike's device-dependent rows (Gate 0A item 1's model/OS/API confirmation, Data Layer pairing, Headless JS wake across foreground/background/Doze, TalkBack/rotary/battery). Path (1) is the cheapest and needs neither new software nor hardware. Until one of these happens, the isolated evidence above is what this environment can produce; it does not substitute for any of the three blocked gates.
...
| Physical-device behavior: React Native startup/navigation/rotary/TalkBack, background/Doze dashboard refresh, Data Layer pairing, battery budgets, phone-handoff notification latency | **Not Tested — Blocked** | No physical device ever connected (`adb devices` empty at every check); two direct pairing attempts (once against a bare AVD, once against a Play-capable AVD with the real companion app present) both confirmed pairing is IDE-mediated with a hard `SecurityException` against CLI activation (spike log above) |
| Minimal native companion probe: build/install/native-entry, both apps, independent of pairing | **Tested** | `spikes/wear-companion/`: both modules build exit 0; both install `Success`; both log native `onCreate` under cold-install and immediate uninstall/reinstall cycles on the bare phone AVD. The earlier "Activity class does not exist" phone-launch failure did not reproduce this pass; a plausible but unconfirmed cause (a different, Play-capable AVD in the prior session) is named, not asserted as proven (spike log above) |
| Local PING/PONG correlation/dedupe logic | **Tested** | 4 local JVM unit tests against the extracted `PingRequestDeduper`, `:watch:testDebugUnitTest` exit 0, 4/4 passed — explicitly not cross-device evidence (spike log above) |
...
- physical Samsung watch evidence (startup, rotary, TalkBack, memory, battery, Data Layer pairing, Headless JS wake) — blocked on device connection, not attempted;
...
- emulator-to-emulator companion Data Layer pairing — confirmed twice by direct attempt (bare AVD, then a Play-capable AVD with the real companion app installed and its automation entry point rejected by `SecurityException`) that pairing is mediated by Android Studio's IDE, unavailable from this CLI-only environment; distinct from physical-device absence, not the same gap restated;
- **resolved 2026-09-23:** the phone-side "Activity class does not exist" launch failure did not reproduce in a fresh diagnostic pass against the same source, on the same bare `orca_phone_api31_e2e` AVD, including an immediate uninstall→reinstall→`am start` chain with no delay — the phone app now launches cleanly every time, logcat-confirmed. The prior failure most likely traced to that earlier session installing onto the Play-capable `orca_phone_api31_play` AVD (heavier system-app set, real Google account, Play Protect) rather than the bare image, not to a defect in the spike's own code; this was not re-tested against that specific AVD given the bound, so it remains a named, unconfirmed environmental hypothesis rather than a proven root cause. The bug that blocked the round trip is closed for the bare-AVD path that will actually be used going forward.
- explicit Dhiman re-confirmation of decisions 2, 10, and 11, and resolution of the still-open decision-register rows.

**What this gate authorizes, and what it does not:** the isolated Phase 0A/0B spike Dhiman authorized has been executed twice now — once against static/build evidence, once attempting an emulator-based companion probe — and produced the evidence above; it does not itself authorize Phase 1 product implementation. Two independent next legitimate asks remain, neither yet satisfied: pairing the phone/Wear emulators through Android Studio's Device Manager (this environment's CLI tooling could then drive the resulting round trip), or connecting a physical Samsung watch. Not blanket approval of the whole product on the strength of the build/code evidence already closed. This plan's zero-waiver completion bar (Definition of done) still requires every paired-machine and every agent-type path proved, not a representative subset, and still requires the extraction-ready-architecture smoke test (already partially closed at the JS/TS layer above; the paired-build phone-counterpart smoke test remains a separate, later prerequisite) to pass before any extraction claim is made. No item in this report is waived by another item passing.
