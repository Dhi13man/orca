...
**Current decision (2026-09-25):** Dhiman chose a standalone watch app that connects directly to Orca while the installed phone app stays unchanged. This supersedes the phone/Data Layer architecture decisions below; those sections are historical design evidence, not current acceptance criteria. Direct enrollment requires an updated Orca runtime, a watch-scoped grant, network reach, and separate enrollment for each runtime. Do not install the obsolete phone bridge build.

**Current acceptance checkpoint:** The direct watch grant, compact dashboard, local/WSL and structured conversation/reply paths, bounded all-pairing fleet inventory, redacted in-app host event feed, and bounded all-provider usage refresh are implemented in source. An unresolved usage fetch returned within 1.5 seconds with an explicit pending flag; the watch schedules at most two short foreground follow-up reads. Short-code manual enrollment uses a 20-character five-minute code to prove the server key before redeeming a Wear-scoped grant. Wrong-code, expiry, replay, and readiness-output tests pass. On 2026-09-25 the x86_64 Wear emulator used its on-watch text entry to enter an endpoint and code, paired with an isolated live encrypted runtime, showed the exact host in Attention, restored the host and a redacted event after app process restart, and showed that host as Unavailable after grant revocation. The optional device test passed (1/1); its Gboard tap coordinates are emulator-specific and physical input remains unverified. Wear 159/159 tests, focused runtime auth/socket 49/49, typecheck/lint, and the ARM64 release build passed. SSH reply source uses exact relay PTY/incarnation writes and a host-side durable receipt ledger; focused reconnect, replay, drift, and Node 18/20/22/24 persistence tests passed. This is **source verification, not a real SSH-host or Watch8 reply**. Relay prebuilt bindings cover Linux/macOS x64/arm64 and Windows x64 on Node 18/20/22; Node 24 uses built-in SQLite. Windows ARM64 before Node 24 and Node 19/21/23 omit the capability and keep SSH Wear send read-only. The watch compiles a narrow FCM intake for generic attention alerts; the APK contains its receiver and notification permission. Firebase project/configuration, watch token upload, a live host sender, user notification permission, and screen-off delivery remain **unimplemented or unverified**, per Dhiman's choice to leave cloud setup pending. Remaining gates include real SSH-host acceptance, background alerts, and physical Watch8 pairing, reply, installed APK identity, and all-fleet acceptance while the device is absent from ADB. Builds and emulator success do not close those gates.
...
**Direct background checkpoint (2026-09-25):** Commit `2a5090503a14774a4b3f2f67ce5cd6e4561b0586` runs a headless direct-watch refresh from the Android 15-minute network job. It checks up to 32 paired hosts with three concurrent reads and a five-second host timeout, writes a 48 KiB maximum encrypted two-slot snapshot, and atomically publishes the completed slot only while its Android run and exact pairing identity are still active. The UI hydrates this snapshot after process restart, labels it stale, shows omitted-agent counts and the 32-host cache limit, and disables cached agent actions until a live read. Android stop aborts outstanding WebSockets and blocks late slot publication; pairing removal or grant replacement invalidates the old snapshot. Unit tests cover stop, in-flight cancellation, restart hydration, replacement, late results, and repeated stale truncation. The exact-commit x86_64 APK (SHA-256 `1023E8609BE2F78EF97F63830395A6DB772E27EB0FEFBCDF5795E69E0D3A606A`) passed the emulator cold-process test (1/1): an isolated host emitted a new terminal-bell event while the app process was killed, a forced job ran, the host grant was revoked, and after a cold app restart the watch still showed the new redacted event while the host showed Unavailable. Wear 169/169 tests, typecheck, lint, format, and the ARM64 release build passed. The ARM64 APK was rebuilt after x86 testing and its hash and single ABI rechecked. This proves one forced-job persistence/UI path, not natural 15-minute timing, screen-off delivery, all-fleet operation, timely alerts, or physical Watch8 installation. FCM project/configuration, watch-token registration, host sender, notification permission, and screen-off push acceptance remain open; cloud provisioning is deferred by Dhiman.

**Push preparation checkpoint (2026-09-25):** Commit `9e3e18de7a217a6bb348b601531637c6f0b8f47c` adds a Wear-only authenticated token-registration RPC and a bounded secure host token store. Revocation removes the token and every send rechecks the active Wear grant. An injected sender receives only a generic allowed event kind and replay ID, never the notification title or body. The sender is not wired to FCM, and the watch does not upload a token while Firebase provisioning is deferred. Node typecheck, scoped lint/format and five focused token/auth tests pass. This is source preparation only: no cloud push, notification permission, screen-off delivery, or physical watch alert was tested. ADB currently lists only emulators. Read-only fleet inventory found Crowley and Choisoonsil OMP sessions active and Rasputin with no sessions; no prompt was sent to ongoing work. Real disposable-session conversation/reply, SSH host, all-fleet and Watch8 acceptance remain open.

**Push review fix:** Read-only review found a same-turn revoke/rotation race between queue validation and deferred send. Commit `6432be19d6a6f8ebae4d36d26d0fcfea46ba91c6` rechecks the grant and exact token immediately before invoking the sender. The new revoke/rotation regression passes with all three push-store tests; Node typecheck, scoped lint and format pass. A send already handed to an external provider cannot be recalled; the sender remains unconfigured here.
