...
**Current acceptance checkpoint:** The direct watch grant, compact dashboard, local/WSL and structured conversation/reply paths, bounded all-pairing fleet inventory, redacted in-app host event feed, and bounded all-provider usage refresh are implemented in source. An isolated encrypted runtime paired with the Wear emulator; the watch displayed Attention, recovered its pairing after app process restart, and replayed a redacted host event. An unresolved usage fetch returned within 1.5 seconds with an explicit pending flag; the watch schedules at most two short foreground follow-up reads. Wear 158/158 tests, runtime rate-limit orchestration 18/18, typecheck/lint, x86_64 emulator flow, and ARM64 release build passed on 2026-09-25. SSH reply source now uses exact relay PTY/incarnation writes and a host-side durable receipt ledger; focused reconnect, replay, drift, and Node 18/20/22/24 persistence tests passed. This is **source verification, not a real SSH-host or Watch8 reply**. Relay prebuilt bindings cover Linux/macOS x64/arm64 and Windows x64 on Node 18/20/22; Node 24 uses built-in SQLite. Windows ARM64 before Node 24 and unsupported odd Node majors omit the capability and keep SSH Wear send read-only. Remaining software gates are usable non-ADB enrollment, real SSH-host acceptance, and timely host-originated background notifications with screen-off evidence. Dhiman chose to prepare FCM code while leaving Firebase cloud setup pending. Physical Watch8 pairing, reply, installed APK identity, and all-fleet acceptance remain unverified while the device is absent from ADB. Builds and emulator success do not close those gates.

**Current ARM64 artifact:** `wear/android/app/build/outputs/apk/release/app-release.apk`, SHA-256 `E6E8959B9985BBD4D70451410D41A2BFB44BF8BA13A53E779FC50680E64FDC88`, signed with the existing debug certificate (`fac61745dc0903786fb9ede62a962b399f7348f0bb6f899b8332667591033b9c`). It is an internal test APK, not a Play/release-signed artifact.
