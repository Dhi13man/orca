...
| Package, signing, version-code, and Play topology                                | Android release owner                | Debug-key fingerprints inspected and confirmed non-meaningful (universal AGP default) 2026-09-23 (spike log below); real release/Play-signed certificate fingerprints and export evidence still required | Open — debug-cert baseline closed, real release evidence outstanding |
...
| Windows SSH atomic-send capability                                               | Dhiman and Orca runtime reviewer     | Approval of revised decision 9; an isolated process-kill probe closed 2026-09-23 found zero corruption across 3 kill points in the documented write/fsync/rename sequence (spike log below) — real crash/power-cut durability evidence, which that probe explicitly cannot provide, **or** Dhiman's explicit approval of a named "all machines" scope reduction excluding Windows SSH hosts | Open — narrow process-kill evidence closed; power-cut proof or scope-reduction decision still pending |
...
Dhiman authorized this isolated feasibility spike. This log records what was actually executed against the real toolchain/codebase — commands, exit codes, and results — not another plan rewrite. No product code was written; no real agent session, paired device, or user workload was touched; the preexisting `.gitignore` modification and untracked `wear/` prototype are unchanged by this pass (verified: `wear/` source files show zero modification by mtime, and Gradle build output this spike generated was removed from the in-place tree — see the toolchain-build item below).
...
**Package/signing/build-config inspection (public certificate fingerprints only — no private key material read).** `keytool -list -v` against both `mobile/android/app/debug.keystore` and `wear/android/app/debug.keystore` (password `android`, the universal AGP default): both report identical SHA-256 `FA:C6:...:9C`. This match is **not meaningful pairing evidence** — it is Android Gradle Plugin's universally-shared default debug key present on every developer machine, not evidence of intentional signing alignment. `mobile/android/app/build.gradle:98,100` confirms `applicationId 'com.stably.orca.mobile'`; `wear/android/app/build.gradle:90,92` confirms `applicationId 'com.dhi13man.orcawear'` — package identities remain mismatched exactly as the earlier evidence re-verification found. `wear/android/app/src/main/AndroidManifest.xml:17` confirms `standalone=true` (plan requires `false`). `wear/android/gradle.properties:63-65` confirms `minSdkVersion=33`, `compileSdkVersion=36`, `targetSdkVersion=36` — this specific SDK floor already matches the plan's Gate 0A build contract, a genuine positive finding independent of the rejected architecture. No real release/Play signing certificate exists anywhere in this checkout (correctly — one should never be committed); that comparison remains unavailable until a real signed build exists.

**Toolchain build probe (Expo/RN/Gradle compile feasibility only — not companion/Data Layer/headless proof, not a device test).** Copied `wear/` into an isolated temp directory (not the preserved prototype), ran `pnpm install --frozen-lockfile` (629 packages, 8.7s) there, then `./gradlew.bat assembleDebug --offline` with a direct, non-piped exit-status capture: `REAL_GRADLE_EXIT_STATUS=0`. Produced a real signed `app-debug.apk` (135,744,237 bytes), independently verified with `apksigner verify --print-certs` (SHA-256 matches the debug keystore above) and `aapt2 dump badging` (`com.dhi13man.orcawear`, versionCode 1, minSdk 33, targetSdk 36). This proves only that JDK 17 + this Android SDK + AGP + Kotlin 2.1.20 + the pinned Expo modules can compile and package this specific source into an installable APK on this machine — it says nothing about the rejected prototype's architecture being acceptable, and nothing about on-device startup, Data Layer pairing, or Headless JS wake, all of which remain unverified. (An initial in-place run of this same command inside the real `wear/android` was corrected after coordinator review: its generated `app/build`, `android/build`, `app/.cxx`, and `.gradle` output were deleted to restore the preserved prototype, and the probe was re-run from the isolated copy with a properly captured exit status.)

**Windows durable-write crash probe (process-kill safety only — explicitly not power-cut proof).** `src/main/durable-file-write.ts:11-13,17-20` documents and its `syncDirectory`/`syncDirectorySync` functions confirm in code that Windows cannot fsync a directory handle; the catch block explicitly treats that as expected. Wrote an isolated Node script (scratch temp directory only, no repo or user data touched) reimplementing the documented write-tmp/fsync/rename/best-effort-dirsync sequence, spawning a child process that self-terminates (`process.exit(137)`) at three boundaries — after write before fsync, after fsync before rename, after rename before directory fsync — then inspecting the destination file from the parent. Result across all three kill points: **zero corruption observed**; the destination was always either the prior complete content or the new complete content, never partial. This is real evidence for "process-kill mid-sequence does not corrupt the destination on this NTFS volume," which is a narrower claim than decision 9's actual bar — a genuine power cut can lose data the OS believed was durable but the disk had not yet persisted, which no software-only probe can simulate. Decision 9 remains open pending real crash/power-cut hardware evidence.
...
| Package/signing identity inspected (public fingerprints) | **Closed** — mismatch confirmed with line evidence; no real release cert available to compare |
| Expo/RN/Gradle toolchain compiles this source | **Closed** (build-only) — real APK produced in isolated copy |
| Extraction-ready standalone build (JS/TS layer) | **Closed** — install/typecheck/test/lint all pass with zero siblings |
| Windows durable-write process-kill safety | **Closed** (narrow claim) — zero corruption in 3/3 probed points |
| Windows durable-write power-cut durability | **Blocked** — no hardware fault-injection available; decision 9 stays open |
| Physical Samsung watch / phone pairing, Data Layer round trip, Headless JS wake, TalkBack, rotary, battery | **Blocked — physical device unavailable** (`adb devices` empty); this blocks the physical gate specifically, not this spike's other findings |
| Real release/Play signing certificate comparison | **Blocked** — no release-signed artifact exists in this checkout |

**Next action:** connect the target Samsung Galaxy Watch (and, for a full companion round trip, a paired Android phone) via `adb`/ADB-over-Bluetooth or Wi-Fi debugging, and re-run this spike's device-dependent rows (Gate 0A item 1's model/OS/API confirmation, Data Layer pairing, Headless JS wake across foreground/background/Doze, TalkBack/rotary/battery). Until then, the isolated evidence above is what Phase 0B can produce; it does not substitute for the physical gates it was never able to reach.
...
| `wear/` compiles into a real, signed, installable APK on this machine's JDK/SDK/Expo toolchain | **Tested** (build-only) | Isolated-copy `assembleDebug --offline`, direct exit-status capture = 0, real APK verified via apksigner/aapt2 (spike log above) — not evidence the rejected architecture or companion mechanism works |
...
| Documented write/fsync/rename sequence survives process-kill at 3 boundary points without destination corruption | **Tested** (narrow) | Isolated Node probe, scratch temp dir only, 3/3 points zero corruption (spike log above) — explicitly not power-cut/hardware-fault evidence |
| Physical-device behavior: React Native startup/navigation/rotary/TalkBack, background/Doze dashboard refresh, Data Layer pairing, battery budgets, phone-handoff notification latency | **Not Tested — Blocked** | `adb devices` empty at spike start and end; no physical or emulator device was paired (spike log above) |
...
- Windows SSH atomic-send crash/power-cut durability (decision 9) — narrow process-kill evidence closed; hardware-level proof still absent, and its interim read-only state is not a substitute for it;
- explicit Dhiman re-confirmation of decisions 2, 10, and 11, and resolution of the still-open decision-register rows.

**What this gate authorizes, and what it does not:** the isolated Phase 0A/0B spike Dhiman authorized has been executed and produced the evidence above; it does not itself authorize Phase 1 product implementation. The next legitimate ask is connecting a physical Samsung watch (and ideally a paired phone) so the blocked device-dependent rows above can be attempted — not blanket approval of the whole product on the strength of the build/code evidence already closed. This plan's zero-waiver completion bar (Definition of done) still requires every paired-machine and every agent-type path proved, not a representative subset, and still requires the extraction-ready-architecture smoke test (already partially closed at the JS/TS layer above; the paired-build phone-counterpart smoke test remains a separate, later prerequisite) to pass before any extraction claim is made. No item in this report is waived by another item passing.
