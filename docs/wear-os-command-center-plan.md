...
| Minimal companion probe: phone-side native entry | **Blocked — new, reproducible bug found**: phone app installs successfully but `am start` fails with "Activity class does not exist" on both first launch and after clean reinstall; not root-caused within this bound (evidence above) |
| Minimal companion probe: PING/PONG round trip, duplicate-ack path | **Blocked** — depends on both the pairing blocker above and the phone native-entry bug above; neither the transport nor the phone-side trigger was reachable this pass |
...
| Minimal native companion probe: build/install/native-entry, independent of pairing | **Tested (partial)** | `spikes/wear-companion/`: both modules built exit 0; watch installed, launched, logcat-confirmed native entry; phone installed but a reproducible "Activity class does not exist" bug blocked its native entry — a real, not-yet-fixed bug, not a fabricated pass (spike log above) |
...
- the phone half of the new minimal companion probe (`spikes/wear-companion/phone`) installs but its `MainActivity` will not launch ("Activity class does not exist"), a real bug blocking the PING/PONG round trip this task asked for even once pairing/hardware exist — not root-caused within this pass's time bound;
