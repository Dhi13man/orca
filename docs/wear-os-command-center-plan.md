...
**Current ARM64 artifact:** `wear/android/app/build/outputs/apk/release/app-release.apk`, SHA-256 `E34C834F765783910C1C3B339247DCEDCCF0B35E89A52C1784AEADD0CACD3A5C`, signed with the existing debug certificate (`fac61745dc0903786fb9ede62a962b399f7348f0bb6f899b8332667591033b9c`). The APK contains only `arm64-v8a` native libraries, on-watch manual enrollment UI, and the compiled FCM intake, but no Firebase project configuration; it is an internal test APK, not a Play/release-signed artifact.
...
**Background delivery gap:** The existing Android periodic job belongs to the obsolete Data Layer route. A direct-pairing experiment on the Wear emulator reached an isolated runtime after process death, but review found that it discarded every dashboard result and could continue requests after Android stopped the job. That experiment was fully reverted before commit, and the ARM64 candidate was rebuilt from committed source with the hash above. Direct-watch background refresh and notifications remain unimplemented. Timely alerts need the deferred Firebase setup and a tested watch-token registration/host sender path; a 15-minute periodic job alone cannot establish alert timing or screen-off delivery.
