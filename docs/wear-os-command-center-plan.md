...
**Current acceptance checkpoint:** The direct watch grant, compact dashboard, local/WSL and structured conversation/reply paths, bounded all-pairing fleet inventory, redacted in-app host event feed, and bounded all-provider usage refresh are implemented in source. An isolated encrypted runtime paired with the Wear emulator; the watch displayed Attention, recovered its pairing after app process restart, and replayed a redacted host event. An unresolved usage fetch returned within 1.5 seconds with an explicit pending flag; the watch schedules at most two short foreground follow-up reads. Short-code manual enrollment now connects the watch to an isolated live runtime without an ADB-delivered pairing link: a 20-character five-minute code proves the server key before the watch sends it, then redeems once for the existing Wear-scoped grant. Wrong-code, expiry, replay, and readiness-output tests pass. The x86_64 Wear emulator installed the current UI; opening Add Orca host from the bottom of the dashboard now resets scroll and shows the endpoint input. Manual keyboard entry and live pairing on emulator or Watch8 remain unverified. Wear 158/158 tests, focused runtime auth/socket 48/48, typecheck/lint, and ARM64 release build passed on 2026-09-25. SSH reply source uses exact relay PTY/incarnation writes and a host-side durable receipt ledger; focused reconnect, replay, drift, and Node 18/20/22/24 persistence tests passed. This is **source verification, not a real SSH-host or Watch8 reply**. Relay prebuilt bindings cover Linux/macOS x64/arm64 and Windows x64 on Node 18/20/22; Node 24 uses built-in SQLite. Windows ARM64 before Node 24 and Node 19/21/23 omit the capability and keep SSH Wear send read-only. The watch compiles a narrow FCM intake for generic attention alerts; the APK contains its receiver and notification permission. Firebase project/configuration, token registration with Orca, host sender, user notification permission, and screen-off delivery remain **unimplemented or unverified**, per Dhiman's choice to leave cloud setup pending. Remaining gates include real SSH-host acceptance, background alerts, and physical Watch8 pairing, reply, installed APK identity, and all-fleet acceptance while the device is absent from ADB. Builds and emulator success do not close those gates.

**Current ARM64 artifact:** `wear/android/app/build/outputs/apk/release/app-release.apk`, SHA-256 `F05C920D6699A583E732E8A40A9BD10C1038CCA1FF382F5BE55C18CC5DCCF5C6`, signed with the existing debug certificate (`fac61745dc0903786fb9ede62a962b399f7348f0bb6f899b8332667591033b9c`). The APK contains only `arm64-v8a` native libraries, manual enrollment UI, and the compiled FCM intake, but no Firebase project configuration; it is an internal test APK, not a Play/release-signed artifact.

**Pairing review gate:** Independent read-only review found and the source now fixes malformed encrypted auth token handling and advertised WebSocket endpoint paths. Wear 159/159 and focused runtime auth/socket 49/49 tests pass. One high-risk output issue remains: human `orca serve --wear-pairing` still prints a long-lived bearer pairing URL beside the five-minute code. A user choice on changing this existing CLI output is pending; do not treat a screenshot or copy of that readiness block as a short-lived credential.
