...
| Provisional target | The user's Samsung Galaxy Watch, provisionally Wear OS 6 but possibly already upgraded to the now-GA Wear OS 7 (Android 17/API 37, released 2026-06-16), plus required round-screen quality profiles; exact model/build is confirmed in Phase 0 |
...
This pass re-read the repository at branch `Dhi13man/ship-orca-wearos`, HEAD `b48130a57a774653fe19ffc14c5885632e9e61e6` (worktree repair evidence and dirty-state record are in the worker report, not this file) and re-ran the primary-source research this plan depends on. Two categories of finding update the register below; neither authorizes product code.

**1. An untracked, unapproved prototype already exists on disk and diverges from every approved decision above.** `wear/` at the repository root (not `mobile/wear/`) is a second, independent Expo app — its own `pnpm-workspace.yaml`/`pnpm-lock.yaml`, disconnected from the `mobile/` workspace this plan requires (Decision 2; evidence: `wear/package.json`, `wear/pnpm-workspace.yaml`). Three concrete divergences, each evidenced by file:line, not inference:

   - **Wrong package/signing identity.** `wear/app.json:18` sets `"package": "com.dhi13man.orcawear"`, not the approved `com.stably.orca.mobile` (Decision 3; phone package confirmed at `mobile/android/app/build.gradle:98-100`). A Data Layer pairing between this watch build and the real Orca phone app cannot succeed — different package families never share a Data Layer node namespace, regardless of Keystore work.
   - **A direct-to-runtime client, not a phone companion.** `wear/src/orca/direct-orca-client.ts`, `wear/src/orca/pairing.ts`, and `wear/src/orca/runtime-rpc-transport.ts` implement E2EE pairing (`wear/src/orca/pairing.ts:8-14`, `scope?: 'mobile' | 'runtime'`) and RPC framing directly against the Orca runtime, with no phone in the path. This is exactly the plan's Phase 4 "conditional later phase" (architecture-alternatives table above) and violates decision 1's companion-first boundary and the explicit rule that "the direct approach must not be introduced as a silent fallback."
   - **A forked design-token set.** `wear/src/wear-theme.ts` defines an independent `wearColors` palette (`background: '#0a0a0a'`, etc.) that does not import `mobile/src/theme/mobile-theme.ts` (`bgBase: '#111111'`, different hex values, different naming). This is the watch token fork the Visual contract section explicitly prohibits.

   This worker did not modify, delete, or otherwise touch `wear/`; the prerequisite instruction to preserve preexisting dirty/untracked files applies. The finding is recorded here because "implementation remains gated by the evidence register" is false in practice while an alternate, non-conforming implementation exists uncommitted on the same worktree. Resolution (archive, discard, or formally reconcile into `mobile/wear/` under the approved architecture) is Dhiman's decision, not a planning default; no option is applied by this pass.

**2. Wear OS 7 is now a general-availability release, not a preview.** The plan's research findings and sources (below) previously classified Wear OS 7 as "preview guidance, not a release dependency" (A2). Re-checked 2026-09-23 against `developer.android.com/training/wearables/versions/7/changes`: Wear OS 7 runs Android 17 (API level 37) and is GA, having shipped 2026-06-16. This does not change the approved API 33 floor (decision 8 still stands as the minimum), but Phase 0's device-inspection step must not assume the watch is on Wear OS 6 — it may already be Wear OS 7, which adds an `ACCESS_LOCAL_NETWORK` runtime-permission requirement for any local-network access (already anticipated, correctly, in Phase 4 item 5) and background-audio foreground-service hardening not otherwise relevant to this plan's non-audio surface. Reclassified A2 → A1 in Sources below.

| Decision                                                          | Owner   | Evidence required before product code                                                                              | State |
| ------------------------------------------------------------------ | ------- | -------------------------------------------------------------------------------------------------------------------- | ----- |
| Reconcile or discard the untracked `wear/` direct-client prototype | Dhiman  | Explicit choice: archive outside the repo, delete, or formally re-scope as an approved Phase 4 spike under `mobile/` | Open  |

Obsolete-parity-design note for the eventual reconciliation decision, recorded now so it is not re-litigated later: `wear/`'s design is a phone-parity pattern this plan rejects everywhere — a standalone app mirroring desktop/phone dashboard concepts (`runtime-dashboard.tsx`) rather than the round-watch-native, phone-companion IA in "Product and interaction design" below. If Dhiman chooses to formally re-scope any part of it into a future Phase 4 spike rather than discard it outright, its direct-client transport, pairing, and theme files are superseded on sight by the approved companion architecture and must be marked for removal in that spike's own plan; this pass does not delete them now, and no other part of this document treats them as a starting point.
...
"ALL paired machines and ALL agent types" is the target surface, not an aspiration statement: every host the user has paired with Orca belongs in the watch's catalog (never a subset), and every terminal-backed and structured agent session the phone itself can see belongs in the watch's agent inventory. Two clarifications keep this bounded rather than unbounded, matching "complete discovery is not unbounded subscriptions or scrolling transcripts":

- **Host catalog completeness vs. live-connection concurrency are different things.** Every paired host is always visible with its cached/stale state; only a small number hold a concurrently *live* socket at once (see the "three-host" clarification under Phase 1 below). No host is ever excluded from the product surface.
- **Agent-type completeness is currently bounded by the phone, not by the watch.** `src/main/runtime/rpc/methods/session-tab-agent-status-projection.ts:16-24` hard-filters structured `agent-session` tabs out of every `session.tabs.subscribeAll`/`listAll` payload whenever `clientKind === 'mobile'` — before any Wear-specific code runs. Orca mobile's own phone screens cannot see structured agent sessions today (matching the existing Non-goals line below). The minimum watch release therefore shows exactly what the phone shows: terminal-backed agents, full parity, no less. Extending coverage to structured agent-session tabs requires a runtime capability change scoped to the `mobile` client kind — a backend/mobile delta outside this feature's extraction boundary (see "Extraction-ready architecture" and the decision register) — not a watch-only workaround, because the watch has no data source the phone does not already have.
...
- Structured `agent-session` tabs, which `session-tab-agent-status-projection.ts:16-24` deliberately removes from every `mobile`-client-kind projection today, not just from mobile's own screens (see "Product framing" above). Add them only with their own capability-gated mobile product surface that also updates Orca mobile's phone screens — the watch inherits phone parity, it does not get ahead of it.
...
- Phone-absent real-time notifications, FCM/relay push infrastructure, complications, or Wear OS 7-specific surfaces beyond the approved API 33 floor (Wear OS 7/API 37 is GA, not preview — see the evidence re-verification above — but it is still out of scope for the minimum release, which targets API 33+).
...
**Extraction-readiness note.** `wear-companion-contract` and `expo-wear-data-layer` (below) are colocated inside this Orca checkout only so the feature can be built against the real mobile/backend stack it depends on; colocation is a workspace-management convenience, not evidence of a decision to publish. If a personal public OSS repository is considered later, it applies only after this plan is fully implemented, reviewed, and Dhiman is satisfied with it — and only after a separate license/provenance/asset audit and explicit approval, none of which this document performs. Nothing here selects a license, prepares a publication, or otherwise acts on that future possibility. The reason this section still matters now is narrower and immediate: `wear-companion-contract` importing no React/Expo/Node/desktop-runtime code (below) and `expo-wear-data-layer` staying a standalone Expo module with no reach into `src/renderer` or other desktop-only trees are also exactly what keeps today's phone/watch boundary honest — a Kotlin/TypeScript boundary that cannot quietly depend on desktop internals is easier to review today, independent of whether it is ever extracted.
...
Design-system pass verification (2026-09-23): `mobile/src/theme/mobile-theme.ts` (73 lines — colors, spacing, radii, typography) is the correct, current incumbent token source and contains no Wear-specific tokens today, so the reuse instruction above has nothing to conflict with yet. `docs/STYLEGUIDE.md`'s resolution order is unaffected — this plan does not define a competing token authority. The five-screen navigation model above (Dashboard, Host, Agent, Compose, Inbox) is round-watch-native rather than a shrunk phone IA: it is organized around attention/agents/usage/handoff, not around the phone's tab structure, and none of it imports `src/renderer/src/assets/main.css` or desktop CSS. The untracked `wear/` prototype (see the Phase 0 evidence re-verification above) is the concrete counter-example this contract exists to prevent: `wear/src/wear-theme.ts` forks its own `wearColors` palette instead of importing `mobile-theme.ts` — cited here as evidence the rule is necessary, not as something this plan endorses or extends.
...
- UI and product logic: the mobile app's pinned Expo SDK, React Native, React, Expo Router, TypeScript, Vitest, oxlint, and oxfmt versions. Today `mobile/pnpm-workspace.yaml` declares no child package members. Phase 0 adds only `wear`, `packages/expo-wear-data-layer`, and `packages/wear-companion-contract`; the repository-root workspace and the existing `mobile/packages/expo-two-way-audio` dependency topology remain unchanged. `mobile/pnpm-lock.yaml` then remains the single dependency lock for the phone root, watch, and two Wear packages, which consume one another through `workspace:*`.
...
- Wear OS 7 (Android 17/API 37) is GA as of 2026-06-16, not a preview lane; it is still not a minimum dependency at the approved API 33 floor, but Samsung upgrade eligibility for the specific target watch remains unconfirmed until Phase 0 reads Settings → About.
...
- A1 — [Wear OS 7 behavior changes](https://developer.android.com/training/wearables/versions/7/changes); re-verified 2026-09-23 as a shipped general-availability release (Android 17/API 37, 2026-06-16), reclassified from A2 preview status. Not a minimum dependency — decision 8's API 33 floor still governs — but the device confirmed in Phase 0 may already be on it.
...
1. Add `mobile/wear/` as a second Expo app and set `mobile/pnpm-workspace.yaml` child members to exactly `wear`, `packages/expo-wear-data-layer`, and `packages/wear-companion-contract`, sharing `mobile/pnpm-lock.yaml`. Do not absorb `mobile/packages/expo-two-way-audio` or change the repository-root workspace as part of this feature. Phone and watch declare the shared Wear packages with `workspace:*`; installs run frozen from the mobile workspace root. Add an automated gate that rejects drift in Expo, React Native, React, TypeScript, Vitest, oxlint, and oxfmt versions between phone and watch. Run Expo Doctor and Expo autolinking verification from both app roots, then assert one resolved copy and peer context for React, React Native, Expo Modules Core, Expo Router, screens, gesture handler, Reanimated, Worklets, and the Wear native module. Duplicate native modules or differing module paths fail the gate.
...
4. Reuse the Phase 0-proven process-level host client owner with a bounded live-socket budget, initially the same 3 concurrently-live hosts mobile's own Home screen already auto-connects (`HOME_AUTO_CONNECT_LIMIT` in `mobile/src/transport/home-host-auto-connect.ts:3`), including the watch-selected host rather than adding a fourth socket. This is a concurrency bound, not a product exclusion: **all** paired hosts remain in the catalog and are selectable regardless of count; hosts outside the live budget show cached/stale state and acquire a slot on selection, exactly as mobile's own `useAllHostClients`/`selectHomeAutoConnectHostIds` pattern already does for the phone. Do not treat the number 3 as fixed — Phase 0's power evidence may justify a smaller watch-specific budget (radio cost is worse on a watch) or, with justified headroom, a larger one; either way it stays a background/off-screen cap, and the on-screen host always holds a slot. A `HostObservationCoordinator` owns one account and notification subscription per acquired host, fans normalized/deduped events to Home and Wear, and never schedules the phone notification twice. Own exactly one `subscribeAll` per acquired host and extend direct/relay cleanup to issue `session.tabs.unsubscribeAll`.
5. Publish only meaningful dashboard changes as one AEAD-encrypted, 24-hour-expiring Data Item per binding. Construct the strict `accountUsage` projection field-by-field before encryption and persistence; negative fixtures prove that account and organization identifiers, runtime targets, inactive accounts, free-form descriptions, buckets, and credits cannot cross the boundary. Host-agent and notification pages are bound/epoch/revision/cursor-fenced, node-targeted transient replies. Every ciphertext envelope has a 32-KiB cap, deterministic ordering, completeness metadata, explicit unavailable/expired failures, and fixtures below Data Layer's 100-KB ceiling.
...
- Allowed usage percentages, window lengths, reset timestamps, status, and freshness match the same active-provider host snapshot rendered by mobile; no other account-snapshot field is serialized.
...
| Agent command center | Host catalog/worktree join covering every paired host, bounded live-socket budget (background cap only, never a catalog exclusion), session inventory, direct/relay cleanup, SSH/folder/Git/old-host tests | Preserve cached/incomplete/unverifiable state                                             |
...
- every currently paired host is catalogued and selectable, not a subset; terminal-backed agents on the small number of concurrently live-socketed hosts (selection always holds a slot; the rest is a background concurrency budget, see Phase 1) render with truthful state across local, SSH, paired-runtime, Git-worktree, and folder-workspace contexts, while other catalog hosts are explicitly cached/stale until selected; structured agent-session tabs match whatever Orca mobile's phone screens already show for the same `mobile` client kind (see "Product framing"), no more and no less, until a separate mobile/runtime decision changes that;
