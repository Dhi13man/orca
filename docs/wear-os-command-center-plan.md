...
The first enrollment run completed its three session tests, then stalled in Android 12 Keystore with `BACKEND_BUSY`; another process stalled immediately. A cold boot of the disposable read-only emulator resolved this and the eight-test suite completed in 0.549 seconds. Android 12's [operation creation implementation](https://android.googlesource.com/platform/frameworks/base/+/android-12.0.0_r1/keystore/java/android/security/KeyStoreSecurityLevel.java) can retry indefinitely; no security or compatibility setting was changed. Product integration must keep crypto off UI threads and bound queued work even when a provider call stalls. Evidence: `spikes/wear-companion/build/product-implementation/android31-enrollment-bindings-cold.log`. Still outstanding: enrollment owner and wire transport, authenticated envelopes, cleanup/ack reconciliation, native inbox, phone ownership, watch screens, runtime actions and installed-device acceptance.
