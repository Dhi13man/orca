...
The phone's pure usage projection now maps only active provider status, session/weekly percentages, reset timestamps, and freshness. It merges Claude by normalized email plus organization UUID and Codex by provider plus workspace account ID only when the rate-limit runtime target selects that managed account; absent or incomplete identities remain separate. The freshest reading records its exact `readingHostId`, and account IDs, email, errors, buckets, reset descriptions, and runtime targets do not enter the projected group. Nine mobile fixtures, mobile typecheck, lint, and independent source review pass. The key resolver still needs a binding-scoped random-key owner; the real host-catalog/process owner, encrypted Data Layer publication, watch rendering, and physical acceptance remain open.
