...
**Current acceptance checkpoint:** The direct watch grant, compact dashboard, local/WSL and structured conversation/reply paths, bounded all-pairing fleet inventory, redacted in-app host event feed, and bounded all-provider usage refresh are implemented in source. An isolated encrypted runtime paired with the Wear emulator; the watch displayed Attention, recovered its pairing after app process restart, and replayed a redacted host event. An unresolved usage fetch returned within 1.5 seconds with an explicit pending flag; the watch schedules at most two short foreground follow-up reads. Wear 158/158 tests, runtime rate-limit orchestration 18/18, typecheck/lint, x86_64 emulator flow, and ARM64 release build passed on 2026-09-25. SSH reply source now uses exact relay PTY/incarnation writes and a host-side durable receipt ledger; focused reconnect, replay, drift, and Node 18/20/22/24 persistence tests passed. This is **source verification, not a real SSH-host or Watch8 reply**. Relay prebuilt bindings cover Linux/macOS x64/arm64 and Windows x64 on Node 18/20/22; Node 24 uses built-in SQLite. Windows ARM64 before Node 24 and Node 19/21/23 omit the capability and keep SSH Wear send read-only. The watch now compiles a narrow FCM intake for generic attention alerts; the generated release APK contains its receiver and notification permission. Firebase project/configuration, token registration with Orca, host sender, user notification permission, and screen-off delivery remain **unimplemented or unverified**, per Dhiman's choice to leave cloud setup pending. Remaining software gates include usable non-ADB enrollment and real SSH-host acceptance. Physical Watch8 pairing, reply, installed APK identity, and all-fleet acceptance remain unverified while the device is absent from ADB. Builds and emulator success do not close those gates.

**Current ARM64 artifact:** `wear/android/app/build/outputs/apk/release/app-release.apk`, SHA-256 `CA95E87C1EC9F1F2BE45B360F2E568D1CB2FB3D4DDD75A9A221BF0306CC29A9A`, signed with the existing debug certificate (`fac61745dc0903786fb9ede62a962b399f7348f0bb6f899b8332667591033b9c`). The APK contains only `arm64-v8a` native libraries and the compiled FCM intake, but no Firebase project configuration; it is an internal test APK, not a Play/release-signed artifact.
