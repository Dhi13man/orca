...
**Current acceptance checkpoint:** The direct watch grant, compact dashboard, local/WSL and structured conversation/reply paths, and bounded all-pairing fleet inventory are implemented in source. An isolated encrypted runtime test and emulator startup passed; the fleet path has focused tests but still needs enrolled emulator acceptance. Remaining software gates are usable watch enrollment, SSH exact-target reply with durable host receipts, host-originated background notifications with screen-off/restart/reconnect evidence, and full Attention/usage freshness behavior. Physical Watch8 pairing, reply, installed APK identity, and all-fleet acceptance remain unverified while the device is absent from ADB. Builds and emulator startup do not close those gates.
